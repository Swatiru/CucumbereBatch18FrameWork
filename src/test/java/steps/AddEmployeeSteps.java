...
public class AddEmployeeSteps extends CommonMethods {
    ...
    public void user_add_multiple_employees_from_datatable_and_verify_they_are_added
        ...
        for (Map<String,String> newEmployee:employeeNames){
            ...
            for (int i=0; i<rowData.size();i++){//till the all the element in row
                System.out.println("i am inside the loop now");
                String rowText = rowData.get(i).getText();// first string i am getting one by one
                //it will print the data from row
                System.out.println(rowText);//print the data for validation
                String expectedData = employeeId + " "+newEmployee.get("firstName")+" "+
                        newEmployee.get("middleName")+" "+newEmployee.get("lastName");//this is second string to validate from first string
                System.out.println(expectedData);
                Assert.assertEquals(rowText, expectedData);// validating first string to second string
            }
            Thread.sleep(2000);
            click(dashboardPage.addEmployeeOption);
        }
